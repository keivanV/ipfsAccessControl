    ...
    def evaluate_node(self, node: AccessTreeNode, CT: Dict, SK: Dict, user_attributes: Set[str], node_id: str, pairing_cache: Dict) -> Optional[Any]:
        if node.type == "leaf":
            ...
            results = [self.evaluate_node(child, CT, SK, user_attributes, node_id, pairing_cache) 
                      for child in node.children]
            if node.gate_type == "AND" and len(results) == len(node.children) and len(results) >= node.threshold and all(results):
...
def run_benchmark():
    ...
    data_sizes = [10240,  51200,  102400 ,512000,  1048576]  
    num_runs = 1
    results = []

    # Scenario 3: Fixed file (1), fixed attributes (2), varying data sizes
    logging.info("Running Scenario 3: Fixed file (1), fixed attributes (2), varying data sizes")
    n_files = 1
    n_attrs = 2
    attributes = [f"attr{i+1}" for i in range(n_attrs)]
    user_attrs = set(attributes)
    for data_size in data_sizes:
        total_chunks = (data_size + cpabe.chunk_size - 1) // cpabe.chunk_size  # Ceiling division
        access_tree = create_access_tree(n_attrs, attributes, "L1")
        policy_str = f"L1: ({' AND '.join(attributes)})"
        scenario = f"Fixed_File_1_Attrs_2_Size_{data_size}"
        ...
        for run in range(num_runs):
            data_list = [get_random_bytes(data_size)]
            ciphertext_list = []
            ...
            logging.info(f"Run {run+1}, Files={n_files}, Chunks={total_chunks}, Attrs={n_attrs}, Size={data_size}: Decryption time = {decrypt_time:.6f}s")
        ...
        if ciphertext_list:
            for key in sizes:
                sizes[key] = sizes[key] // len(ciphertext_list)
        ...
        for key in sizes:
            sizes[key] = sizes[key] if success_count > 0 else 0

        logging.info(f"Scenario 3: Files={n_files}, Chunks={total_chunks}, Attrs={n_attrs}, Size={data_size}, Success Rate={success_rate:.2f}%")
    ...
    csv_headers = ['Scenario', 'Files', 'Chunks', 'Attributes', 'Data Size (Bytes)', 'Policy', 'Setup Time (ms)', 'Keygen Time (ms)', 
    ...
    if tabulate:
        print_performance_tables(data_sizes, results)
    else:
        print("\nScenario 3: Fixed File (1), Fixed Attributes (2), Varying Data Sizes:")
        for data_size in data_sizes:
            try:
                result = next(r for r in results if r['Scenario'] == f"Fixed_File_1_Attrs_2_Size_{data_size}")
                print(f"Files={result['Files']}, Chunks={result['Chunks']}, Attrs={result['Attributes']}, Size={data_size}, "
                      f"Encrypt={result['Encrypt Time (ms)']:.6f}ms, Decrypt={result['Decrypt Time (ms)']:.6f}ms, "
                      f"Success={result['Success Rate (%)']:.1f}%")
            except StopIteration:
                pass

def print_performance_tables(data_sizes: List[int], results: List[Dict]) -> None:
    if not tabulate:
        return
    print("\nScenario 3: Fixed File (1), Fixed Attributes (2), Varying Data Sizes:")
    table = [['Files', 'Chunks', 'Attributes', 'Data Size (Bytes)', 'Policy', 'Setup (ms)', 'Keygen (ms)', 'Encrypt (ms)', 
              'Decrypt (ms)', 'MPK (B)', 'MSK (B)', 'SK (B)', 'CT (B)', 'Success (%)']]
    for data_size in data_sizes:
        try:
            result = next(r for r in results if r['Scenario'] == f"Fixed_File_1_Attrs_2_Size_{data_size}")
            table.append([
                result['Files'], result['Chunks'], result['Attributes'], data_size, result['Policy'],
